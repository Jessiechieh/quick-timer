...
var start, isBlink, isLight, isRun, isShow, isWarned, handler, latency, stopBy, delay, audioRemind, audioEnd, newAudio, soundToggle, setExplosion, formatTime, show, adjust, toggle, reset, blink, count, run, resize;
start = null;
isBlink = false;
isLight = true;
isRun = false;
...
setExplosion = function(state){
  return document.body.classList.toggle('explode', state);
};
formatTime = function(ms){
  var totalMs, minutes, seconds, milli;
  totalMs = Math.max(0, ms);
  minutes = Math.floor(totalMs / 60000);
  seconds = Math.floor(totalMs % 60000 / 1000);
  milli = Math.floor(totalMs % 1000);
  return minutes + ":" + (seconds < 10 ? "0" : "") + seconds + ":" + (milli < 100 ? "0" : "") + (milli < 10 ? "0" : "") + milli;
};
show = function(){
  isShow = !isShow;
